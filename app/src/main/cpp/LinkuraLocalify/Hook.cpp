...
#pragma region FreeCamera
    ...
    DEFINE_HOOK(void, Unity_set_fieldOfView, (UnityResolve::UnityType::Camera* self, float value)) {
        ...
        if (self == mainCameraCache) {
            Log::DebugFmt("self: Unity_set_fieldOfView HOOKED");
        }
    ...
    DEFINE_HOOK(float, Unity_get_fieldOfView, (UnityResolve::UnityType::Camera* self)) {
        // never be called
        if (self == mainCameraCache) {
            Log::DebugFmt("self: Unity_get_fieldOfView HOOKED");
        }
        if (Config::enableFreeCamera && (L4Camera::GetCameraSceneType() != L4Camera::CameraSceneType::NONE)) {
            if (mainCameraCache) {
                Unity_set_fieldOfView_Orig(mainCameraCache, L4Camera::baseCamera.fov);
    ...
    DEFINE_HOOK(void, Unity_set_rotation_Injected, (UnityResolve::UnityType::Transform* self, UnityResolve::UnityType::Quaternion* value)) {
        // never be called
        if (self == cameraTransformCache) {
            Log::DebugFmt("self: Unity_set_rotation_Injected HOOKED");
        }
        if (Config::enableFreeCamera && (L4Camera::GetCameraSceneType() != L4Camera::CameraSceneType::NONE)) {
            if (cameraTransformCache) {
    ...
    DEFINE_HOOK(void, Unity_set_position_Injected, (UnityResolve::UnityType::Transform* self, UnityResolve::UnityType::Vector3* data)) {
        // never be called
        if (self == cameraTransformCache) {
            Log::DebugFmt("self: Unity_set_position_Injected HOOKED");
        }
        if (Config::enableFreeCamera && (L4Camera::GetCameraSceneType() != L4Camera::CameraSceneType::NONE)) {
            if (cameraTransformCache) {
                const auto cameraMode = L4Camera::GetCameraMode();
    ...
    // register with meets main camera but will effect the with meets
    ...
    DEFINE_HOOK(void, CameraManager_RemoveFixedCamera, (Il2cppUtils::Il2CppObject* self, Il2cppUtils::Il2CppString* name, void* method)) {
        if (Config::enableFreeCamera) unregisterMainCamera();
        Log::DebugFmt("CameraManager_RemoveFixedCamera HOOKED");
        CameraManager_RemoveFixedCamera_Orig(self, name, method);

    }

    // fes live only
    // register fes live fixed camera as main camera
    // 但是需要在各种set position 函数判断是否为当前cameraCache，要不然会被复位
    ...
    // story only
    // 需要 set position 函数判断是否为当前cameraCache，要不然会被复位
...
#pragma region FreeCamera_ADD_HOOK
        ...
        ADD_HOOK(CameraManager_RemoveFixedCamera, Il2cppUtils::GetMethodPointer("Core.dll", "Inspix", "CameraManager", "RemoveFixedCamera"));
